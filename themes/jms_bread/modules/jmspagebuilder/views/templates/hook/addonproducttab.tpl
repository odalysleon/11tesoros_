...
<div class="jms-tab1">
	...
	{$cf = 0}
		{if $config.show_featured eq '1'}
			<li class="nav-item"><a class="button active" data-toggle="tab" href="#featured">{l s='Featured' d='Modules.JmsPagebuilder'}</a></li>
		{$cf = $cf + 1}
		{/if}	
		{if $config.show_new eq '1'}
			<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#latest">{l s='New Arivals' d='Modules.JmsPagebuilder'}</a></li>
			{$cf = $cf + 1}
		{/if}		
		{if $config.show_topseller eq '1'}
			<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#topseller">{l s='Best Seller' d='Modules.JmsPagebuilder'}</a></li>
			{$cf = $cf + 1}
		{/if}		
		{if $config.show_special eq '1'}
			<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#special">{l s='Special' d='Modules.JmsPagebuilder'}</a></li>
			{$cf = $cf + 1}
		{/if}			
		{if $config.show_onsale eq '1'}
			<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#onsale">{l s='On Sale' d='Modules.JmsPagebuilder'}</a></li>
			{$cf = $cf + 1}
		{/if}			
	</ul>
</div>
