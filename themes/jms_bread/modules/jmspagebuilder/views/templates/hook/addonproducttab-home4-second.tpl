...
var tab2_itemsDesktop = {if $cols}{$cols|escape:'htmlall':'UTF-8'}{else}4{/if};
var tab2_itemsDesktopSmall = {if $cols_md}{$cols_md|escape:'htmlall':'UTF-8'}{else}3{/if};
var tab2_itemsTablet = {if $cols_sm}{$cols_sm|escape:'htmlall':'UTF-8'}{else}2{/if};
var tab2_itemsMobile = {if $cols_xs}{$cols_xs|escape:'htmlall':'UTF-8'}{else}1{/if};
</script>
		...
		<ul class="nav nav-tabs" role="tablist">
			{$cf = 0}
				{if $config.show_featured eq '1'}
					<li class="nav-item"><a class="button active" data-toggle="tab" href="#featured2">{l s='Featured' d='Modules.JmsPagebuilder'}</a></li>
					{$cf = $cf + 1}
				{/if}	
				{if $config.show_new eq '1'}
					<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#latest2">{l s='New Arivals' d='Modules.JmsPagebuilder'}</a></li>
					{$cf = $cf + 1}
				{/if}		
				{if $config.show_topseller eq '1'}
					<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#topseller2">{l s='Best Seller' d='Modules.JmsPagebuilder'}</a></li>
					{$cf = $cf + 1}
				{/if}		
				{if $config.show_special eq '1'}
					<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#special2">{l s='Special' d='Modules.JmsPagebuilder'}</a></li>
					{$cf = $cf + 1}
				{/if}			
				{if $config.show_onsale eq '1'}
					<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#onsale2">{l s='On Sale' d='Modules.JmsPagebuilder'}</a></li>
					{$cf = $cf + 1}
			{/if}			
		</ul>
	</div>
