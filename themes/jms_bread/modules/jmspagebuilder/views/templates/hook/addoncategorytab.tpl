	...
	<div class="jms-tab1">
		<ul class="nav nav-tabs" role="tablist">
			{foreach from = $categories key = k item = category}
				<li class="nav-item"><a class="button {if $k == 0}active{/if}" data-toggle="tab" href="#category-{$category.id_category|escape:'html':'UTF-8'}">{$category.name|escape:'htmlall':'UTF-8'}</a></li>					
			{/foreach}	
		</ul>
	</div>
</div>
